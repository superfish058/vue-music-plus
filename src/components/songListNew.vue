				...
				<div v-for="item,index in songList" :key="index" class="songItem " @dblclick="playMusic(item.id)" v-tap="(e)=>playMusic(item.id)">
					...
					<el-col :span="11" class="width90 mobileSize">
						<div class="songbox index_1">
							<div class="songImg">
								<el-image :src="item.album.picUrl+'?param=200y200'" fit="fill" @click="playMusic(item.id,0)"
									lazy>
								</el-image>
							</div>
					...
					<el-col :span="2" class="PC">
						<div class="flexBox ">
							<span>{{setDtime(item.duration)}}</span>
						</div>
					</el-col>
				</div>
			</div>
