			...
			<el-col :span="!showMobileLyric ? 12 : 24">
				...
				<el-row class="mobile">
					...
					<el-row class="songName">
						...
						<span class="iconfont icon-shipin mvIco" v-if="currentMusicInfo.mv" :style="{ color: themeColor }"
							@click="turnMvPage(currentMusicInfo.mv)"></span>
					</el-row>
					<el-row class="songInfo">
						<span v-for="(item, index) in currentMusicInfo.artist.split('/') || currentMusicInfo.artist"
							:key="index" style="margin-right: 5px;cursor: pointer;max-width: 45vw;" class="hover noWrop"
							@click="turnSingerPage(currentMusicInfo.singerIds[index])">
							{{ item }}
						</span>
						<span style="cursor: default;"> - </span>
						<span @click="turnAlbumPage(currentMusicInfo.alId)" style="cursor: pointer;max-width: 45vw;"
							class="hover noWrop">
				...
				<el-row class="mobile mobileBottom">
					...
					<div class="center">
						<div class="funcMobile">
							<div class="left">
								<span class="iconfont icon-shixin" style="font-size: 30px;" @click="addSong()"
									v-if="!currentIn"></span>
								<span class="iconfont icon-shixin" :style="{ fontSize: '30px', color: themeColor }"
									@click="removeSong()" v-else></span>
								<i class="el-icon-chat-dot-round hover" :style="{ fontSize: '24px' }"
									@click="turnCommentPage()"></i>
								<i class="el-icon-brush" :style="{ fontSize: '24px', color: themeColor }"
									@click="setColor = !setColor"></i>
							</div>
							<div class="right">
			...
			<el-col :span="!showMobileLyric ? 12 : 24" class="lyricArea" :key="currentId" v-show="!showMobileLyric">
				...
				<el-row class="songInfoArea">
					<p class="onlyOne">
						<!-- 歌手 -->
						<span v-for="(item, index) in currentMusicInfo.artist.split('/') || currentMusicInfo.artist"
							:key="index" style="margin-right: 5px;cursor: pointer;" class="hover"
							@click="turnSingerPage(currentMusicInfo.singerIds[index])">
							{{ item }}
						</span>
						<span style="cursor: default;"> - </span>
